...
export default function Weather() {
  ...
  const fetchWeather = useCallback(async () => {
    setLoading(true);
    try {
      // Use first field with coordinates or default location
      const fieldData = company?.id ? await getFields(company.id) : [];
      setFields(fieldData);
      const refField = fieldData.find((f) => f.latitude && f.longitude);
      const lat = refField?.latitude || -25.75;
      const lng = refField?.longitude || 28.19;
      ...
      <MotionBox initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
          <Box>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              Operational weather analysis for drone spray operations
              {lastRefresh && ` • Updated ${lastRefresh.toLocaleTimeString('en-ZA', { hour: '2-digit', minute: '2-digit' })}`}
            </Typography>
          </Box>
          <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchWeather} disabled={loading}>
            Refresh
          </Button>
        </Box>
        ...
        <Grid container spacing={3}>
                ...
                <Paper sx={{ p: 3, bgcolor: '#FFFFFF', height: '100%' }}>
                  ...
                  {weather && (
                    <>
                      ...
                      <Typography sx={{ fontSize: '0.9rem', color: 'text.secondary', textTransform: 'capitalize', mb: 3 }}>{weather.description}</Typography>
          ...
          <Grid item xs={12} lg={4}>
            ...
            {/* Field Status */}
            {fields.filter((f) => f.latitude).length > 0 && (
              <Paper sx={{ p: 3, bgcolor: '#FFFFFF' }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  {fields.filter((f) => f.latitude).slice(0, 5).map((field) => {
                    ...
                    return (
                      <Box key={field.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1.5, borderRadius: '8px', border: '1px solid rgba(15,23,42,0.04)' }}>
                        <Box>
